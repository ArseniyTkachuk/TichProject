<template>
    <div class="test-wrapper">
        ...
        <header class="test-header">
            ...
            <h1 class="test-title">Редагувати тест</h1>
        ...
        <div v-for="(q, qIndex) in exercises" :key="qIndex" class="question-block">
            ...
            <div v-if="q.type === 'one' || q.type === 'many'" class="answers-block">
                <div v-for="(a, aIndex) in q.answers" :key="aIndex" class="answer-row">
                    ...
                    <label>
                        ...
                        <template v-else>
                            <input type="radio" :name="'one-' + qIndex" :value="aIndex"
                                v-model="q.correctAnswerIndex" /> Правильна
                        </template>
                    </label>
            ...
            <div v-if="q.type === 'pair'" class="pairs-block">
                <div class="pair-columns">
                    <div>
                        <h6>Ліва колонка</h6>
                        <div v-for="(left, lIndex) in q.pairs.left" :key="'left-' + qIndex + '-' + lIndex"
                            class="pair-row">
                            <input v-if="!left.isImage" v-model="left.text" placeholder="Лівий елемент"
                                class="pair-input" />
                    ...
                    <div>
                        <h6>Права колонка</h6>
                        <div v-for="(right, rIndex) in q.pairs.right" :key="'right-' + qIndex + '-' + rIndex"
                            class="pair-row">
                            <p>{{ rIndex + 1 }}</p>
                            <input v-if="!right.isImage" v-model="right.text" placeholder="Правий елемент"
                                class="pair-input" />
        ...
        <div class="btn-down">
            ...
            <button @click="updateTest" class="btn-create-test">Зберегти зміни</button>
        </div>
...
export default {
    ...
    methods: {
        async fetchTest() {
            try {
                const res = await api.get(`/getOneTest/${this.testId}`, {
                    headers: { Authorization: `Bearer ${localStorage.getItem("tokenAuthTeacher")}` }
                });
                const test = res.data.test;
                this.title = test.title;
                this.exercises = test.exercises.map(q => {
                    let correctAnswerIndex = null;
                    if (q.type === "one") {
                        correctAnswerIndex = q.answers.findIndex(a => a.correct) ?? null;
                    }
                    return {
                        ...q,
                        correctAnswerIndex,
            ...
            try {
                ...
                this.exercises.forEach((q, qIndex) => {
                    (q.answers || []).forEach((a, aIndex) => {
                        if (a.isImage && a.file) formData.append(`images[q${qIndex}][a${aIndex}]`, a.file);
                    });
                    if (q.type === "pair") {
                ...
                const res = await api.patch(`/test/${this.testId}`, formData, {
